...
#+begin_src elisp
  (use-package org-super-links
    :quelpa (org-super-links :repo "toshism/org-super-links" :fetcher github :commit "0.5")
    :bind (("C-c s s" . org-super-links-link)
         ("C-c s l" . org-super-links-store-link)
         ("C-c s C-l" . org-super-links-insert-link)))
#+end_src
...
- Emacs 30.1+ compatibility: fix timestamp formatting (#106)
- Don't directly use the value of =org-time-stamp-format= (#78)
- Add capture target buffer to search targets (#68)
- Fix byte-compile warnings (#54)
- =org-super-links-link= argument is now optional (#104)

Breaking changes
- =advice-add= for =org-capture= auto-store is no longer added automatically (#101). Users who relied on a link being stored whenever =org-capture= was called must now add the advice to their own configuration:
  #+begin_src emacs-lisp
  (advice-add 'org-capture :before #'org-super-links-store-link)
  #+end_src

Bugfixes
- Handle target buffers potentially being narrowed (#73)
- Keep the original narrowing states (#76)
- Fix bug in =org-super-links-related-into-drawer=
