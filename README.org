...
The package includes a test suite to ensure functionality works correctly. Tests can be run in several ways:

*** Interactive Testing in Emacs

You can run the tests interactively within Emacs:

#+begin_src elisp
M-x ert-run-tests-interactively RET org-super-links-test
#+end_src

This opens a buffer showing detailed test results.

*** Command Line Testing

For automated testing or CI/CD pipelines, run:

#+begin_src bash
emacs -batch -l ert -l org-super-links.el -l org-super-links-tests.el -f ert-run-tests-batch-and-exit
#+end_src

*** GitHub Actions

The repository includes GitHub Actions workflows that automatically run tests on push and pull requests. These tests run against multiple Emacs versions (27.1, 28.2, 29.1, and latest snapshot) to ensure compatibility.

*** Writing New Tests

When fixing bugs or adding features, please add appropriate tests to =org-super-links-tests.el=. The existing tests provide examples of how to structure new tests.
